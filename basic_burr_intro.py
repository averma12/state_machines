...
def human_input(state: State) -> Tuple[dict, State]:
    ...
    return {"prompt": prompt}, state.append(chat_history=chat_item)
...
def ai_response(state: State) -> Tuple[dict, State]:
    ...
    content = call_llm_simple(system_prompt="You are a helpful assistant.", user_prompt=state["chat_history"])
